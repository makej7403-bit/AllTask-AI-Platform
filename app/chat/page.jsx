...
export default function ChatPage(){
  ...
  async function send(){
    ...
    setMsg('');
    try{
      const res = await fetch('/api/ai', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ message: userMsg })});
  ...
  return (
    <div>
      ...
      <div className="card" style={{minHeight:240,maxHeight:420,overflow:'auto'}}>
        ...
        {history.map((m,i)=>(<div key={i} style={{marginBottom:8}}><div style={{fontWeight:700}}>{m.role==='user'?'You':'FullTask AI'}:</div><div>{m.text}</div></div>))}
      </div>
